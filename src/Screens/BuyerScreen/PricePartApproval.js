...
const BuyerId= localStorage.getItem('userId');
    ...
    // console.log("helloooooooooooooooooooooo",userId);
    ...
    //     var tempbuyer = [];
    //     axios.get(AxioxExpPort + "buyer/all_buyer").then((res)=> {
    //         console.log("buyerresponse", res.data);
    //         res.data.map((item,index)=>{
    //             tempbuyer.push({
    //                 ...item,item.Name:item.address
    //             })
    //         })
    //           console.log("buyerdata", tempbuyer);
    //           setSelectBuyer(tempbuyer);
    ...
    //   "EVENT_END_DATE": dateFormat(endDate, "dd-mm-yyyy"),);
      ...
      const dateS=dateFormat(startDate, "yyyy-mm-dd")
        if (currentStep === 4) {
            //  await axios.post(`${AxioxExpPort}part_price/add?id=${selectBuyer[0]}`,
          await axios
            .post(
              `${AxioxExpPort}part_price/add?id=${BuyerId}`,

              {
                ...
                EVENT_END_DATE: dateE.toString(),
                supplier: vendorIds, // Use the correct variable name
                rfq: storeArr.RFQNO,
              }
            )
            .then((res) => {
